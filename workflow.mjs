...
<style>:root{font-family:"Segoe UI","Microsoft YaHei",Arial,sans-serif;color:#20252b;background:#f5f7f9;line-height:1.5}*{box-sizing:border-box}body{margin:0}.shell{max-width:900px;margin:0 auto;padding:28px 24px 56px}.topbar{display:flex;justify-content:space-between;gap:20px;align-items:flex-end;border-bottom:1px solid #d9e0e6;padding-bottom:22px}.eyebrow{font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#64717e}.title{margin:4px 0 0;font-size:30px;line-height:1.15}.nav{display:flex;gap:8px;margin-top:12px}.link,.save-button,.browse-button{font-size:12px;color:#155d91;text-decoration:none;border:1px solid #bfd1df;border-radius:4px;padding:7px 11px;background:#fbfdff;cursor:pointer;white-space:nowrap}.save-button{background:#155d91;color:#fff;border-color:#155d91}.browse-button:hover,.link:hover{background:#edf5fb}.browse-button:disabled{opacity:.6;cursor:wait}.card{margin-top:20px;background:#fff;border:1px solid #dfe5ea;border-radius:6px;padding:20px}.card h2{margin:0 0 8px;font-size:20px}.card p{color:#52606b;font-size:13px}.field{display:block;margin-top:16px;font-size:13px;color:#37434d}.path-row{display:flex;gap:8px;align-items:center;margin-top:6px}.path-input{display:block;flex:1;min-width:0;border:1px solid #cbd4dc;border-radius:4px;padding:10px;font:inherit}.actions{display:flex;align-items:center;gap:12px;margin-top:22px}.message{font-size:12px;color:#52606b}@media(max-width:600px){.path-row{align-items:stretch;flex-direction:column}.browse-button{width:100%}}</style></head><body><main class="shell"><header class="topbar"><div><div class="eyebrow">CAREER-OPS / SETTINGS</div><h1 class="title">导出设置</h1><nav class="nav"><a class="link" href="index.html">岗位总览</a><a class="link" href="profile.html">个人资料</a><a class="link" href="settings.html">导出设置</a></nav></div><div class="eyebrow">本地配置</div></header>
<form id="settings-form"><section class="card"><h2>PDF 和 Word 导出路径</h2><p>保存简历时，工作流会在项目输出目录保留一份，并按设置复制一份到这里。可以手动输入路径，也可以点击“选择文件夹”。留空则只保存到工作流输出目录。</p><label class="field">PDF 导出路径<div class="path-row">${input('pdfExportDir', settings.pdfExportDir)}<button class="browse-button" type="button" data-pick-directory="pdfExportDir">选择文件夹</button></div></label><label class="field">Word 导出路径<div class="path-row">${input('wordExportDir', settings.wordExportDir)}<button class="browse-button" type="button" data-pick-directory="wordExportDir">选择文件夹</button></div></label><div class="actions"><button class="save-button" type="submit">保存设置</button><span class="message" id="message">修改后点击保存</span></div></section></form></main><script>(function(){const form=document.querySelector('#settings-form');const message=document.querySelector('#message');for(const button of form.querySelectorAll('[data-pick-directory]'))button.addEventListener('click',async()=>{button.disabled=true;message.textContent='正在打开系统文件夹选择窗口…';try{const response=await fetch('/__workflow/pick-directory',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({field:button.dataset.pickDirectory})});const data=await response.json();if(!response.ok||!data.ok)throw new Error(data.error||'无法选择文件夹');if(data.cancelled){message.textContent='已取消选择';return}form.elements[button.dataset.pickDirectory].value=data.path;message.textContent='路径已填入，请点击“保存设置”';}catch(error){message.textContent='选择失败：'+(error.message||error)}finally{button.disabled=false}});form.addEventListener('submit',async(e)=>{e.preventDefault();const body={};for(const field of form.querySelectorAll('input'))body[field.name]=field.value.trim();message.textContent='正在保存…';try{const response=await fetch('/__workflow/settings/save',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const data=await response.json();if(!response.ok||!data.ok)throw new Error(data.error||'保存失败');message.textContent='设置已保存';}catch(error){message.textContent='保存失败：'+(error.message||error)}})})();</script></body></html>`;
}
