...
- **User Layer (NEVER auto-updated; personalization goes HERE):** `workflow-input/profile/`, `workflow-input/personal-info/personal-info.md`, `workflow-input/jd/`, `workflow-input/photos/`, `cv.md`, `config/profile.yml`, `modes/_profile.md`, `modes/_custom.md`, `article-digest.md`, `portals.yml`, `data/*`, `documents/*`, `reports/*`, `output/*`, `interview-prep/*`
...
- `workflow-input/profile/` (preferred modular profile) · `workflow-input/personal-info/personal-info.md` (compatibility export) · `cv.md` (legacy fallback) · `article-digest.md` · `config/profile.yml` · `modes/_profile.md` · `writing-samples/`
...
- **Examples:** `Run career-ops workflow mode`, `Run career-ops workflow/pdf mode`, `Run career-ops scan mode`, `Run career-ops pipeline mode for data/pipeline.md`, `Run career-ops pdf mode`, `Run career-ops tracker mode`, `Evaluate this JD with career-ops auto-pipeline: https://company.com/jobs/123`
...
| Wants to process `data/job-inbox/` as the personal batch workspace | `workflow` — profile gate + JD intake + evaluation + HTML CVs + cumulative summary |
| Wants a PDF from a confirmed workflow CV | `workflow/pdf` — explicit PDF-only step |
...
- `workflow-input/profile/` is the preferred modular source of candidate facts; `workflow-input/personal-info/personal-info.md` is its compatibility export
- `cv.md` in project root remains the legacy CV fallback
