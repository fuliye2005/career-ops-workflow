...
推荐使用 `profile/` 模块化资料库：每个项目或经历是一个容器，容器内的每条能力、工作要点或项目成果可以单独设置 `familiarity: 1-5`。详细格式见 `workflow-input/profile/README.md`。

workflow 会自动根据 JD 计算匹配度，并按“个人熟悉度 40% + JD 匹配度 60%”排序。岗位总览页会展示两项分数和综合分；模块化资料还会自动生成兼容旧流程的 `personal-info.md`。
