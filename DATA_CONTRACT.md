...
The public workflow input scaffold lives under `workflow-input/`. Prefer the modular candidate profile in `workflow-input/profile/`; the workflow can generate the compatibility `workflow-input/personal-info/personal-info.md` export. Put one job URL per line in `workflow-input/jd/jobs.txt`, place JD screenshots and other attachments beside that file, and keep optional resume photos in `workflow-input/photos/`. These inputs are local user data and must not be committed except for the checked-in placeholder files.
